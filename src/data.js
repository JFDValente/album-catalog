const catalog = [
  {
    id: 1,
    title: 'A Voz, O Violão, A Música De Djavan',
    artists: ['Djavan'],
    thumbnail: 'https://img.discogs.com/0KUKvvg-Z0QtrIbxBltsr6g6x7w=/fit-in/398x400/filters:strip_icc():format(jpeg):mode_rgb():quality(90)/discogs-images/R-1222933-1305645396.jpeg.jpg',
    released: 1976,
    country: 'Brazil',
    genre: 'Latin',
    style: 'MPB',
    trackList: ['Flor de Lís', 'Na Boca Do Beco', 'Maçã Do Rosto', 'Para-Raio', 'E Que Deus Ajude', 'Quantas Voltas Dá Meu Mundo', 'Maria Das Mercedes', 'Muito Obrigado', 'Embola Bola', 'Fato Consumado', 'Magia', 'Ventos Do Norte']
  },
  {
    id: 2,
    title: 'A Voz, O Violão, A Música De Djavan',
    artists: ['Djavan'],
    thumbnail: 'https://img.discogs.com/0KUKvvg-Z0QtrIbxBltsr6g6x7w=/fit-in/398x400/filters:strip_icc():format(jpeg):mode_rgb():quality(90)/discogs-images/R-1222933-1305645396.jpeg.jpg',
    released: 1976,
    country: 'Brazil',
    genre: 'Latin',
    style: 'MPB',
    trackList: ['Flor de Lís', 'Na Boca Do Beco', 'Maçã Do Rosto', 'Para-Raio', 'E Que Deus Ajude', 'Quantas Voltas Dá Meu Mundo', 'Maria Das Mercedes', 'Muito Obrigado', 'Embola Bola', 'Fato Consumado', 'Magia', 'Ventos Do Norte']
  },
];

export default catalog; 